...
const ServiceCard = ({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-lg shadow-lg transition-all duration-500 ${isFeature ? 'transform md:-translate-y-4' : ''
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-b ${isFeature
            ? 'from-primary/80 to-primary/95'
            : 'from-black/60 to-black/90'
          } transition-all duration-300 ${isHovered ? 'opacity-90' : 'opacity-80'
          }`}></div>
      </div>
      ...
      <div className="relative z-10 p-8 md:p-10 h-full flex flex-col">
        {/* Icon */}
        <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 ${isFeature ? 'bg-secondary text-primary' : 'bg-white/10 text-white'
          }`}>
          <i className={`fas fa-${icon} text-2xl transition-transform duration-300 ${isHovered ? 'transform rotate-12' : ''
            }`}></i>
        </div>

        {/* Title & Description */}
        <h3 className={`text-2xl md:text-3xl font-bold mb-4 ${isFeature ? 'text-white' : 'text-white'
          }`}>
          {title}
        </h3>

        <p className={`mb-6 ${isFeature ? 'text-white/90' : 'text-white/80'
          } flex-grow`}>
          {description}
        </p>
        ...
        <div className="mt-auto">
          <Link
            href={linkUrl}
            className={`inline-flex items-center text-sm font-semibold ${isFeature
